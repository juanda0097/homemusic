...
<body> 
    <section class="content">
        ...
        <h2 style="text-align:center; color:blue;">Consulta Album, Casa Musical y canciones</h2>
        ...
        <table width="100%" border="1" style="margin: 0 auto;" id="tabla">
            ...
            <tbody>
                @if($consultapdf3->count())
                @foreach($consultapdf3 as $consulta)   
                <tr>
                    <td>{{ $consulta->nameSong1}}</td>
                    <td>{{ $consulta->duration}}</td>
                    <td>{{ $consulta->nameAlbum}}</td>
                    <td>{{ $consulta->date}}</td>
                    <td>{{ $consulta->namehome}}</td>
